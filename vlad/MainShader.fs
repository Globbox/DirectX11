...
{ 
    float height =  texture(gHightMap, texCoords).r;    
    vec2 p = viewDir.xy / viewDir.z * (height * 0.6);
    return texCoords - p;    
} 
...
{                                                                                            
	...
	//vec3 Vpos = normalize( TBN * gEyeWorldPos);
 // vec3 Fpos = normalize(TBN * WorldPos0);
	
	                                                                                    
    vec3 VertexToEye = (gEyeWorldPos - WorldPos0); 
	vec2 NewTexCoord0 =  ParallaxMapping(TexCoord0,VertexToEye);                                                                                    
    vec4 SampledColor = texture2D(gColorMap, NewTexCoord0.xy);                                  
